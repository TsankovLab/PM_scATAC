...
# Load RNA
srt = readRDS (file.path('..','..','Endothelial','scrna','srt.rds'))
...
top_genes=20
head (degClusters[degClusters$cluster == 'COL4A1',], top_genes)
degClusters = degClusters[degClusters$avg_log2FC > 0,]
degClusters = degClusters[degClusters$p_val_adj < 0.05,]
top_deg = degClusters %>% arrange(p_val_adj) %>% group_by (cluster)  %>% slice(1:top_genes)
top_deg$gene[top_deg$cluster == 'COL4A1']

# Filter TF that are higher expressed in fetal endo
if (!exists('mSE')) mSE = fetch_mat(archp, 'Motif')
TFs = rowData (mSE)$name
fetal_TF = degClusters$gene[degClusters$cluster == 'COL4A1'][degClusters$gene[degClusters$cluster == 'COL4A1'] %in% TFs]
...
all (rownames(endo_modules) == rownames(archp@cellColData))
fetal_groups_sample = lapply (sams, function(sam) 
  data.frame (
    fetal_group = as.character(cut(
  endo_modules$fetal.COL4A1[archp$Sample == sam],
  #breaks = quantile(endo_modules$fetal.COL4A1[archp$Sample == sam], probs = c(0, 1/4, 2/4,3/4, 1)), # Tertiles
  breaks = quantile(endo_modules$fetal.COL4A1[archp$Sample == sam], probs = c(0, 1/2, 1)), # Tertiles
  #labels = c("low", "mid1",'mid2', "high"),
  labels = c("low", "high"),
  include.lowest = TRUE)),
  barcode = rownames(archp@cellColData)[archp$Sample == sam]))
fetal_groups_sample = do.call (rbind, fetal_groups_sample)

archp$fetal_group = NA
archp$fetal_group[match(fetal_groups_sample$barcode, rownames(archp@cellColData))] = fetal_groups_sample$fetal_group
...
## Generate scatterplot of fetal and adult TA vs GS without smoothing ####
if (!exists('fSE')) fSE = fetch_mat(archp, 'scATAC_datasets')
fMat = assays (fSE)[[1]]
rownames (fMat) = gsub ('rawlins_fetal_lung_','', rownames (fMat))
...
df <- data.frame (
  ...
  #MEF2C = mMat$MEF2C[archp$Sample == sam][cells],
...
p1 <- ggplot(df, aes(x = fetal_ta, y = fetal_gs, color = sample)) +
  ...
  #   size = 2, color = "black"
  # ) +
  facet_wrap (~sample, scales = 'free', ncol=length(sams), strip.position = "top") +
  gtheme_no_rot +
  # theme(
...
p2 <- ggplot(df, aes(x = fetal_ta, y = fetal_gs, color = sample)) +
 ...
 gtheme_no_rot +
  ...
  scale_color_manual(values = palette_sample)  # Regression line with confidence interval
# Print the plot
pdf (file.path ('Plots',paste0('adult_vs_fetal_scatterplot_smoothed.pdf')), width=9, height=4)
...
gsMats = t(scale (gsMat[rownames(gsMat) %in% active_TFs,]))
cor_l=list()
cor_gs_l = list()
for (sam in sams)
{
library (zoo)
bin_width <- 20   # Number of observations per bin
overlap <- bin_width
frip = archp$FRIP
fscore_order = modMat$fetal.COL4A1[archp$Sample == sam]
fscore_order = order (fscore_order)
...
smoothed_gsMat = rollapply (gsMats[archp$Sample == sam,][fscore_order,], width = bin_width, FUN = mean, by = overlap, partial = TRUE, align = "left")
print (dim (smoothed_mMat))
cor_l[[sam]] = as.data.frame (cor (smoothed_mMat, fetal_gs, method='pearson'))
cor_l[[sam]]$sample = sam
cor_l[[sam]]$gene = rownames(cor_l[[sam]])
cor_l[[sam]]$type = 'activity'
cor_gs_l[[sam]] = as.data.frame (cor (smoothed_gsMat, fetal_gs, method='pearson'))
cor_gs_l[[sam]]$sample = sam
cor_gs_l[[sam]]$gene = rownames(cor_gs_l[[sam]])
cor_gs_l[[sam]]$type = 'genescore'
}

top_genes=20
cor_df = do.call (rbind, cor_l)
cor_gs_df = do.call (rbind, cor_gs_l)
cor_df = rbind (cor_df, cor_gs_df)
TF_genescore_order = cor_df[cor_df$type == 'genescore',] %>% group_by(gene) %>% summarise(median_value = median(V1))
TF_activity_order = cor_df[cor_df$type == 'activity',] %>% group_by(gene) %>% summarise(median_value = median(V1)) 
TF_activity_order = TF_activity_order[TF_activity_order$gene %in% TF_genescore_order$gene,]
TF_order = data.frame (gene = TF_activity_order$gene, mean = rowMeans (cbind (TF_activity_order[,'median_value',drop=F],TF_genescore_order[,'median_value',drop=F])))
TF_order = head (TF_order %>% arrange (-mean), top_genes)
#TF_order = head (TF_order[TF_order$gene %in% fetal_TF,] %>% arrange (-mean), top_genes)

cor_df = cor_df[cor_df$gene %in% TF_order$gene,]
cor_df$gene = factor (cor_df$gene, levels = rev (unique(TF_order$gene)))
  bp = ggplot (cor_df, aes (x = V1, y = gene, fill = type), alpha=.5) + 
...
# Also check expression in scrna ####
pdf (file.path ('Plots','fetal_endothelial_tfs.pdf'))
DotPlot (srt, features = TF_order$gene, group.by = 'celltype2') + gtheme
dev.off()


# Show TF as scatterplot with genescore and activity and size as RNA expression in fetal endo ####
## Check RNA fetal markers ####
logfcThreshold = 0.5
Idents (srt) = 'celltype2'
degClusters = FindAllMarkers (srt, max.cells.per.ident = 1000, min.pct = .1, logfc.threshold = logfcThreshold, verbose = T)
...
cor_df = do.call (rbind, cor_l)
colnames(cor_df)[1] = c('activity')
cor_gs_df = do.call (rbind, cor_gs_l)
colnames(cor_gs_df)[1] = c('genescore')
cor_df = cbind (cor_df, cor_gs_df[rownames(cor_df),'genescore',drop=F])
cor_df = na.omit(cor_df)
cor_df <- cor_df %>%
  group_by(gene) %>%
  summarise(
    avg_activity = mean(activity, na.rm = TRUE),
    avg_genescore = mean(genescore, na.rm = TRUE)
  )


cor_df$LFC_fetal = degClusters$avg_log2FC[match(cor_df$gene, degClusters$gene)]
cor_df$LFC_fetal_sign = as.character(sign (cor_df$LFC_fetal))
cor_df$LFC_fetal = abs (cor_df$LFC_fetal)
cor_df$LFC_fetal[is.na(cor_df$LFC_fetal)] = 0
cor_df$alpha = rowMeans  (cor_df[,c('avg_activity', 'avg_genescore')])
cor_df$labels = ''
cor_df$labels[match (head (cor_df$gene[order(-cor_df$alpha)],15),cor_df$gene)] = head(cor_df$gene[order(-cor_df$alpha)],15)
cor_df$islabel = cor_df$labels != ''
  bp = ggplot (cor_df, aes (x = avg_activity, y = avg_genescore,
     label = labels)) + 
  geom_point (aes (
    alpha = alpha, 
    fill = LFC_fetal_sign,
    ...
    color = islabel), 
    shape = 21) + 
  scale_fill_manual (values  = c('-1'= 'grey', '1'='purple')) + 
