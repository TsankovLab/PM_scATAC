...
celltypes = unique (srt$shared_cnmf2_r_max)
...
pseudobulk_counts = log2(as.data.frame (AverageExpression (srt, 
...
hg38_genes_in_bins = split(hg38_genes$symbol[subjectHits(genes_in_bins)], queryHits(genes_in_bins))
pseudobulk_counts_mean = lapply(hg38_genes_in_bins, function(x) 
  colSums(pseudobulk_counts[x,, drop=F]))
pseudobulk_counts_mean = do.call (rbind, pseudobulk_counts_mean)
colnames (pseudobulk_counts_mean) = paste0(colnames(pseudobulk_counts_mean),'_scrna')
...
#colnames(elementMetadata (windows))[14] = 'Monocytes_scatac'

# Bin atac fragments ####
metaGroupName2 = unique(archp$Sample)
...
common_celltypes = c('Monocytes','IM','SPP1','TREM2','cDCs','C1Q','IFN')
...
for (tf in km2)
  {
  tf_peaks = rowRanges(tf_match)[rowSums (assay(tf_match[,tf])) > 0,]
  rna = as.data.frame (elementMetadata (windows)[,paste0(common_celltypes,'_scrna')][subjectHits(findOverlaps (tf_peaks, windows)),])
  rna[is.na(rna)] = 0
  atac = as.data.frame (elementMetadata (windows)[,paste0(common_celltypes,'_scatac')][subjectHits(findOverlaps (tf_peaks, windows)),])
  atac[is.na(atac)] = 0
  #atac = atac[!rowSums(atac) == 0 & rowSums(rna) == 0, ]
  #rna = rna[!rowSums(atac) == 0 & rowSums(rna) == 0, ]
  message (paste0('compute cor for ', tf))
  tf_cor[[tf]] <- mapply(function(x, y) cor(x, y, method = 'spearman'), 
    as.data.frame(t(rna)), 
    as.data.frame(t(atac)))
  tf_cor[[tf]][is.na(tf_cor[[tf]])] = 0
   #= cor (t(rna), t(atac), method = 'spearman')
  }
tf_median = sapply (tf_cor, function(x)summary(x)[4])
tf_median = tf_median[order(tf_median)]
tf_cor_df = do.call (rbind, tf_cor)
tf_cor_df = tf_cor_df[order(tf_cor_df[,1]),]
