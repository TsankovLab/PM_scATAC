...
mp2hit <- gr_bed(file.path(OUT,"Average65_mp3_hits.bed"))
mp2 <- ps[unique(queryHits(findOverlaps(ps, mp2hit)))]
cat(sprintf("foreground Average65_mp3 peaks: %d ; background all peaks: %d\n", length(mp2), length(ps)))
...
write.csv(head(tb, 200), file.path(OUT,"Average65_mp3_GREAT_GO_BP.csv"), row.names=FALSE)
...
write.csv(tc, file.path(OUT,"Average65_mp3_GREAT_sarc_custom.csv"), row.names=FALSE)
