...
scr_m, scr_n = panel(zs.loc[genes].T.T, ncell)
panels = [("CROP-seq screen", LINE_COL["CROP-seq"], scr_m, scr_n, "cells")]
scr_order = list(scr_m.columns)                                     # NTC + screen KOs in screen clustering order
shared = [k for k in scr_order]
for cl in THREE:
    g = group_log2avg(cp10k, meta, cl).loc[genes]
    cols = [k for k in scr_order if k in g.columns]                 # only KOs present in the screen, same order
    g = g[cols]
    z = g.sub(g.mean(axis=1), axis=0).div(g.std(axis=1, ddof=1), axis=0).fillna(0.0)
    cnt = meta[meta.Cell_Line == cl]["Gene_Control"].value_counts()
    panels.append((f"{cl} bulk (RUN1)", LINE_COL[cl], z[cols], cnt.reindex(cols), "reps"))
print("bulk columns:", cols)
...
fig.text(0.05, 0.960, "rows = the screen's 5000 variable genes present in bulk, same order in every panel (clustered on the CROP-seq screen)", fontsize=8.5, color=INK2)
fig.text(0.05, 0.946, "columns = KO groups present in the screen, in the CROP-seq panel's order (bulk has no MEF2A/MEF2D); values = log2(mean CP10K + 1), z-scored per gene across the KO groups shown", fontsize=8.5, color=INK2)
fig.text(0.05, 0.932, "number in brackets = cells (screen) or replicates (bulk). NCI-H28 excluded; outlier samples removed (RUN1).", fontsize=8.5, color=INK2)
...
fig.savefig(f"{FIG}/fig1_side_by_side_CROPseq_vs_3lines_RUN1_matchedKOs.png", dpi=220, facecolor="white")
fig.savefig(f"{FIG}/fig1_side_by_side_CROPseq_vs_3lines_RUN1_matchedKOs.pdf", facecolor="white")
print("saved")
